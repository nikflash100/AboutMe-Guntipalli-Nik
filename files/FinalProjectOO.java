...
public class FinalProjectOO {
    ...
    // Question #13
    public List<Map.Entry<String, Integer>> top5Words(Set<String> stopWords) {
        Map<String, Integer> freq = new HashMap<>();
        for (String word : words) {
            if (stopWords != null && stopWords.contains(word)) continue;
            freq.put(word, freq.getOrDefault(word, 0) + 1);
    ...
    public static void main(String[] args) {
        Set<String> stopWords = Set.of("the", "a", "an", "in", "of", "at", "to", "and", "is", "it");
        FinalProjectOO project = new FinalProjectOO("Book.txt");

        if (project.countTotalWords() == 0) {
            System.out.println("no words loaded, check Book.txt");
            return;
        }

        List<String> longWords = project.filterWordsByLength(7);

        System.out.println("Q1 Total words: " + project.countTotalWords());
        System.out.println("Q2 Unique words: " + project.countUniqueWords());
        System.out.println("Q3 Most common word (excluding stopwords): " + project.mostCommonWord(stopWords));
        System.out.println("Q4 Words >= 7 letters: " + longWords.subList(0, Math.min(10, longWords.size())));
        ...
        System.out.println("Q13 Top 5 words: " + project.top5Words(stopWords));
        System.out.println("Q14 Chapters: " + project.countChapters());
        System.out.println("Q15 Anagrams: " + project.findAnagrams());
        System.out.println("Q16 Phrase count: " + project.countPhrase("Seaweed Brain"));
    }
}
